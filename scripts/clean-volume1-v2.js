/**
 * Script de nettoyage Volume 1 Endobiogénie (Version 2 - fichier txt)
 *
 * RÈGLES STRICTES:
 * - Ne résume RIEN
 ...
 * 5. Supprimer watermarks (lignes avec email)
 */
...
const SOURCE = path.join(__dirname, '../RAG/endobiogénie/La Theorie de l\'Endobiogenie VOL1- - Kamyar M. Hedayat.txt');
const DEST = path.join(__dirname, '../RAG/endobiogénie/volume1_final_v2.md');
...
// Watermark à supprimer
const WATERMARK_PATTERN = /Ce document est la propriété exclusive de .* - .* à \d{2}h\d{2}/;
...
function isWatermark(line) {
  return WATERMARK_PATTERN.test(line);
}

function cleanVolume1V2() {
  console.log('=== NETTOYAGE VOLUME 1 ENDOBIOGÉNIE (V2) ===\n');
  ...
  const SKIP_END_INTRO = 970;   // Fin des pages de garde + sommaire (Chapitre 1 commence ligne 971)
  const INDEX_START = 26303;    // Début de l'index alphabétique (avant ligne "Index")
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isWatermark(line)) {
      watermarksRemoved++;
      // Remplacer le watermark par une ligne vide pour ne pas casser le texte
      line = '';
    }
